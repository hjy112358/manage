    ...
    layui.use(['jquery', 'table', 'layer', "form", "layedit", "laydate"], function () {
        ...
        tableIns = table.render({
            ...
            cols: [[
                ...
                { field: 'StockBillEntry_BatchNo', title: '批号', edit: 'text' },
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (isussecc) {
                    ...
                    $("#StockBill_Salesman").html(html);
                    $(".checkper .layui-anim.layui-anim-upbit").html(htmlsel);
                    renderForm1()
                    var select = 'dd[lay-value="' + mouser + '"]';
                    $('#StockBill_Salesman').siblings("div.layui-form-select").find('dl').find(select).click();
                } else {
                    alert(res.Message)
                }
            }
                        ...
                        if (isussecc) {
                            if(data.Children){
                                ...
                                $.each(data.Children, function (index, value) {
                                   ...
                                   value.StockBillEntry_Unit=value.AssignEntry_Unit
