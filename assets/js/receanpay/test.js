...
layui.use(['jquery', 'table', 'layer', "form", "layedit", "laydate", "upload"], function () {
    ...
    var isenter = true;
    // 客户
    $(".changeinput").on("propertychange change  input paste", function () {
                    ...
                    if (isussecc) {
                        ...
                        if (data.length == 0) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                if (i == 0) {
                                    html += '<li data-id="' + datanow.F_Id + '" class="active" data-area="' + area + '" data-rate="' + rate + '" data-nick='+datanow.Customer_Nick +'><p>' + (datanow.Customer_Name || '') + '</p><p>' + (datanow.Customer_Nick || '') + '</p></option>';
                                } else {
                                    html += '<li data-id="' + datanow.F_Id + '" data-area="' + area + '" data-rate="' + rate + '" data-nick='+datanow.Customer_Nick +'><p>' + (datanow.Customer_Name || '') + '</p><p>' + (datanow.Customer_Nick || '') + '</p></option>';
                                }
                            }
    ...
    // 弹窗数据
    getablelist = function (url, type, tableTitle, clickElem,isfirst) {
    ...
    $(".searchupdata").on("click", function () {
        ...
        getablelist(url, type, title, clickelem,1)
       
    })
    $("#param").keydown(function (event) { //键盘上下键以及回车键
        var keycode = (event.keyCode ? event.keyCode : event.which);
        if (event.keyCode == 13) {
            var param = $("#param").val()
            console.log(param)
